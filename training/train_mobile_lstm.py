...
from datagenerator import VideoClasses, FeaturesGenerator, FeaturesGeneratorTwoStream
from model_lstm import lstm_build
from model_i3d import load_model_without_topLayer,lstm_model

from time import time

def train_i3d_LSTM_model(rgbFeatureDir:str, oflowFeatureDir:str, i3d_MODEL_DIR:str, sModelDir:str,
                         sLogPath:str, classesDir, classes_num=35,nBatchSize:int=16, 
                         nEpoch:int=100, fLearn:float=1e-4, load_preModel=False, dropout=0.8):
    
    #i3dModel = load_model_without_topLayer(i3d_MODEL_DIR)
    if (not load_preModel):
        lstmModel = lstm_model((4,1,1,2048),classes_num,dropout)
    else:
        lstmModel = load_model("./gdrive/My Drive/LSTM_i3d/results/model/-1545742658-last.h5")
        
    oClasses = VideoClasses(classesDir)
    
    train_feature_generator(rgbFeatureDir, oflowFeatureDir, sModelDir, sLogPath, lstmModel, oClasses, nBatchSize=nBatchSize,
                           nEpoch=nEpoch, fLearn=fLearn)

def train_feature_generator(rgbFeatureDir:str, oflowFeatureDir:str, sModelDir:str, sLogPath:str,
                            keModel:keras.Model, oClasses:VideoClasses,nBatchSize:int=16, nEpoch:int=100, fLearn:float=1e-4):

    # Load training data
    genFeaturesVal   = FeaturesGeneratorTwoStream(rgbFeatureDir + "/val", oflowFeatureDir + "/val", nBatchSize,
        keModel.input_shape[1:], oClasses.liClasses)
    genFeaturesTrain = FeaturesGeneratorTwoStream(rgbFeatureDir + "/train", oflowFeatureDir + "/train", nBatchSize, 
        keModel.input_shape[1:], oClasses.liClasses)

    # Helper: Save results
    csv_logger = keras.callbacks.CSVLogger(sLogPath.split(".")[0] + f"-{int(time())}-acc.csv")

    # Helper: Save the model
    os.makedirs(sModelDir, exist_ok=True)
    checkpointLast = keras.callbacks.ModelCheckpoint(
        filepath = sModelDir + "/" + (sLogPath.split("/")[-1]).split(".")[0] + f"-{int(time())}-last.h5",
        verbose = 0)
    checkpointBest = keras.callbacks.ModelCheckpoint(
        filepath = sModelDir + "/" + (sLogPath.split("/")[-1]).split(".")[0] + f"-{int(time())}-best.h5",
...
if __name__ == '__main__':
    ...
    #gdrive/My Drive/oflow_results/model/
    
    train_i3d_LSTM_model(rgbFeatureDir="RGB_featuresdataset/",
                         oflowFeatureDir="featuresdataset/",
...
#20181023-0930-chalearn035-oflow-i3d-entire-best_downloaded.h5