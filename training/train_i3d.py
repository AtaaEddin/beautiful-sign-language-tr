...
def train_I3D_RGB_end2end(diVideoSet):
    ...
    * saves logs and models to disc.
    ...
    sClassFile       = "gdrive/My Drive/turkish_sign_language/turkish_20classes.csv" 
    ...
    sOflowDir        = "lol"#"data-temp/%s/%s/oflow"%(diVideoSet["sName"], sFolder)
    #sOflowFeatureDir = "data-temp/%s/%s/oflow-i3d"%(diVideoSet["sName"], sFolder)
    
    sModelDir        = "gdrive/My Drive/rgb_results/model"
    ...
    # Load training data
    genFramesTrain = FramesGenerator(sImageDir + "/train", nBatchSize, 
        diVideoSet["nFramesNorm"], 224, 224, 3, oClasses.liClasses)
    genFramesVal = FramesGenerator(sImageDir + "/val", nBatchSize, 
        diVideoSet["nFramesNorm"], 224, 224, 3, oClasses.liClasses)

    sModelPath = sModelDir + "/20181224-1600-chalearn035-rgb-i3d-entire-best.h5"
    keI3DOflow = keras.models.load_model(sModelPath)
    ...
    sLog = time.strftime("%Y%m%d-%H%M", time.gmtime()) + \
        "-%s%03d-rgb-i3d"%(diVideoSet["sName"], diVideoSet["nClasses"])
    
    # Helper: Save results
    csv_logger = keras.callbacks.CSVLogger("gdrive/My Drive/rgb_results/log/" + sLog + "-acc.csv", append = True)
...
def train_I3D_oflow_end2end(diVideoSet):
    ...
    * saves logs and models to disc.
    ...
    #sImageFeatureDir = "data-temp/%s/%s/image-i3d"%(diVideoSet["sName"], sFolder)
    sOflowDir        =  "oflowdataset"#"data-temp/%s/%s/oflow"%(diVideoSet["sName"], sFolder)
    #sOflowFeatureDir = "data-temp/%s/%s/oflow-i3d"%(diVideoSet["sName"], sFolder)
    
    sModelDir        = "gdrive/My Drive/oflow_results/model"
    ...
    sModelPath = sModelDir + "/20181225-0950-chalearn035-oflow-i3d-entire-best_0.5_0.94_same.h5"
    keI3DOflow = keras.models.load_model(sModelPath)   
    """
    # Load pretrained i3d model and adjust top layer 
    ...
    # Helper: Save results
    csv_logger = keras.callbacks.CSVLogger("gdrive/My Drive/oflow_results/log/" + sLog + "-acc.csv", append = True)
    ...
    keI3DOflow = tf.contrib.tpu.keras_to_tpu_model(
    keI3DOflow,
    ...
    keI3DOflow.fit_generator(
        ...
        callbacks=[csv_logger, cpTopLast, cpTopBest])#
    """
    #keI3DOflow.save_weights(sModelDir + "/" + sLog + "tpu_wieghts.h5")
