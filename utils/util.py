...
def handler(vid_dir,
	...
	if pred_type == "word":
		...
		predictions = sent_preds(rgbs,oflows,frame_num if not from_worker else data['frame_num'],labels,lstmModel,rgb_model,oflow_model,
...
def csv_to_dict(labels_dir,sWord_col):
	df = pd.read_csv(labels_dir,encoding = 'utf-8')
