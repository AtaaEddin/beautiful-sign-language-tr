...
"""
def cal_oflow(arPrev, arCurrent, sAlgorithm="tvl1"):

    arFlow = None
     

    oTVL1 = cv2.DualTVL1OpticalFlow_create(
                scaleStep = 0.5, warps = 3, epsilon = 0.02)
    print("before")
    if sAlgorithm == "tvl1":
        arFlow = oTVL1.calc(arPrev, arCurrent, None)
    elif sAlgorithm == "farnback":
        arFlow = cv2.calcOpticalFlowFarneback(arPrev, arCurrent, flow=None, 
            pyr_scale=0.5, levels=1, winsize=15, iterations=2, poly_n=5, poly_sigma=1.1, flags=0)
    else: raise ValueError("Unknown optical flow type")
    print("after")
